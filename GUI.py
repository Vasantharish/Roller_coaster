...
file.close() 
model = load_model('Roller.keras')#loading model
...
def prediction(face_):
    ...
    age  = round(val[1][0][0])
    color = (0,255,255) if age>13 and age < 60 else (255,0,0)
...
options = ['video.mp4',"video1.mp4","video2.mp4"] #options for testing custom videos
        ...
        while cap.isOpened():
            ...
            if status:
                ...
                for idx, f in enumerate(face):                # loop through detected faces
                    ...
                    label = f'age: {age} gender: {gender}' if age > 13 and age < 60 else f'Not Allowed'
